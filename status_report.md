...
### Newly Discovered UI Bugs (Post-Fix Refactor) — ALL FIXED ✅

#### 1. ~~Missing Visual Feedback~~ — **FIXED ✅**
**Fix:** Re-added `.assigned` CSS class for strikethrough visual feedback while keeping checkboxes enabled. `Nx` badge also shown.

#### 2. ~~No Drag-to-Move Between Tables~~ — **FIXED ✅**
**Fix:** Added `draggable="true"` to assigned chips + HTML5 `dragstart`/`dragover`/`drop` event listeners on drop zones for inter-table chip movement.
...
## QA & Unit Testing Results (2026-04-23)

### Security Audit Findings
1. **Path Traversal (High Risk)**: The `/{filename:path}` endpoint in `server.py` allows Local File Inclusion, resolving paths like `../../` outside of `BASE_DIR`.
2. **SQL Injection Vulnerabilities (High Risk)**: Discovered hardcoded SQL string generation and f-string usages for `INSERT`, `UPDATE`, `UPSERT`, and `SELECT` statements (e.g., `server.py` lines 481, 498, 501, 565), rendering the app vulnerable to injection via dynamic table names and mapping config values.
3. **Local Exposure**: The `uvicorn.run` host is statically set to `0.0.0.0` which exposes the development backend unrestrictedly.
4. **Cross-Site Scripting (XSS)**: Errors generated by the server via `friendlyError` may be unsafely evaluated as HTML nodes into DOM elements during toast messages in `app.js`.

### Automated Test Findings
- The `test_*.py` files execute successfully, successfully performing data connections, API pings, and queries without explicit Python crashing.
- However, standard unit-testing assertions are completely missing; outputs verify successful Python module executions and table-printing but lack `assert` boundaries or testing frameworks like `pytest` and `unittest`.
- Static analysis testing (`bandit`) flags `B608:hardcoded_sql_expressions` and `B104:hardcoded_bind_all_interfaces`.

### Recommended Future Work (Code Currently Frozen)
- Refactor SQLAlchemy query bindings inside `server.py` to prevent SQL Injections.
- Employ `pytest` to strictly enforce and assert data ingestion, mapping outputs, and transactional functionalities automatically.
- Restrict directory traversal checks on static file resolutions.
